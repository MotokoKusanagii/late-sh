...
- **On a short terminal the events move into the rail instead** (`ui::events_in_rail`, under 24 rows). The full-width strip is the better shape when there are rows to spend on it - log lines are sentences, and sentences want width - but a phone held sideways is twenty rows, and there `height / 3` comes off the one thing that cannot be scrolled back to: the live field. Below the threshold the field takes the full height and `draw_log_strip` is handed the bottom third of the side rail (`ui::rail_log_height`, which always leaves the room summary the larger share), a column of wrapped text either way. `draw_room_side` is untouched by this: it measures its click rects against whatever rect it is handed, so the split happens in `draw_game` above it.
...
- `Map`: two pages on one key, chosen by `state::MapMode`. `Field` is the graphical overhead world map (§5.1); `Lands` is the land map (§5.2). `map_fits` is **32x10** (`MAP_MIN_W`/`MAP_MIN_H`) - the floor the map *body* needs, not the chrome around it, which `MapChrome` sheds instead (§5.1). `lands_fit` is still 76x12, because the land picture is a fixed 76x13 drawing that cannot be shrunk; below that the page is not offered at all rather than degraded (see `m` under Active game keys). The text atlas in the side panel is what is left under 32x10, a thin band above compact mode.
...
- `Crafting`: recipes worked at the craft station(s) in the room; select and Enter to craft. **On terminals ≥ 100x20 it expands to a full-screen master-detail layout** (`draw_craft_screen`, the shop's and the pack's third twin): one line a recipe on the left (what it makes, a readiness cell, the `▲/▼` tag) under the collapsible skill headers, and on the right the piece it would make stood against what is worn in its slot through the same `ItemDetail` the shop uses, then the **ingredient checklist** (`✓`/`✗` per line with what is in the pack against what it costs), the trade gate (`Smithing 12 · needs 8 · +70 xp`), and what Enter will do. The readiness cell is the one thing a maker scanning the list wants and it sits where the shop puts its price: `ready`, `lvl N` for a trade gate, or `1/2` for the ingredient lines covered. The sidebar `crafting_panel` serves anything smaller and carries the output's stat line too, because a name plus a pile of ingredients never once said what came out of them. Entries arrive **best-first within each trade** (by `Item::power`, then level gate), sorted per station so the sections stay in station order.
...
- **terrain key** first (the biome colours are already on the map): needs 60 columns and 20 rows.
- **marker legend** (18 rows) and **symbol legend** (16 rows), both also gated on `MAP_LEGEND_W` (92 columns). Width sheds the legends first and hardest: they are single unwrapped ~90-column lines, so on a narrow terminal they were never a legend - they were a sentence clipped mid-word at the edge, costing a row of map to say `known, el`.
- **the inspector's second row** (what is at the crosshair) below 12 rows. On the surviving single row a `heading` outranks the region name, and with no heading the region name outranks the list of what is in the room.
- **header, the inspector's first row and the controls line stay to the floor**: between them they say where you are, what you are pointing at, and how to move the view. The controls line itself picks one of four widths (`ui::map_controls_line`) rather than being chopped - the full line is 79 columns, so on a phone it used to end at `x mark destina`, losing the two keys a lost player most needs. Each arm fits the floor of its own band, pinned by `the_map_sheds_its_legends_before_its_body_on_a_phone`.
...
- `CraftEntryView` carries what the output **is**, not just its name (rarity, stats, desc, slot, `compare`/`compare_pct`/`worn_*`, and the ingredient lines itemised as `CraftIngredientView { name, need, have }`), because a craft is a gear decision the way a purchase is and the screen stands it against what is worn with the same `ItemDetail` the shop uses. The itemised ingredients are what let the screen name the material you are short of; `craftable`/`reason` alone could only say "need materials" and send a maker back to the shops guessing.
...
- **An item's stat line says what using it does, never what selling it pays.** `ItemKind::Utility` - every weapon oil and poison in the game - used to render as `sell 12g`, the one number that has nothing to do with using it, so a shelf of Sparkseed, Chillrime and Blessed Oil said nothing about what separated them. `Item::coat_effect` now reads the very constants the service applies in `use_item`/`coat_weapon` (`svc::OIL_PER_TICK`/`OIL_CHARGES`, `POISON_PER_TICK`/`POISON_CHARGES`) and states the school, the rider and the window: `coat a weapon: +10 fire a strike for 12 strikes`. Deriving it rather than authoring it is what stops the shelf promising a number combat does not deliver. Cooked meals do the same for their lasting half (`+5 hp a tick for 8 ticks`), through `items::food_well_fed` - which is also what `svc::use_item` applies, so the number the pack promises and the number the tick grants are one place. Pinned by `a_coat_says_which_school_it_adds_and_for_how_long_not_what_it_sells` and `a_cooked_meal_says_the_regen_it_leaves_behind_as_well_as_the_heal`.
...
Saved character schema version: `21`.
...
- ability-bar order as ability ids (`ability_order`; schema v21, `#[serde(default)]`, empty means the natural unlock order, resolved through `abilities::ordered_for`).
