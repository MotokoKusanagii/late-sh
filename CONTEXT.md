...
- **Cross-session fan-out goes through `LISTEN/NOTIFY`,** not through a process-local `broadcast`. Shape to copy: `shop_user_changed`, `pot_changed`, `crown_changed`, `bonsai_changed` (a listener task seeds a process-shared `watch`, sessions read the `watch`).
...
| `late-ssh/src/app/door/lateania/CONTEXT.md` | Lateania top-level screen, landing/launch/leave/reset behavior, active-world key capture, game runtime, world/content, combat, classes, abilities, items, wildlife, Frontier, persistence, or game UI panels. | Single Lateania context: screen lifecycle, module map, gameplay loop, service/runtime model, world and content invariants, combat/economy rules (the world map moved to `PROGRESSION.md`, next row), save schemas, tests, and gotchas. |
| `late-ssh/src/app/door/lateania/PROGRESSION.md` | Where a Lateania land sits, what it drops, or what opens it: balance passes, new zones, loot tiers, gate titles. | The world map: every land's level band, median health, xp rate and gear tier; the fourteen crowns; the gate spine; the connection graph; and the measured power ladder to read before re-tuning any tier. Owns those numbers outright - the Lateania `CONTEXT.md` keeps no copy. |
...
| `late-ssh/src/app/door/minecraft/CONTEXT.md` | The Minecraft card in the Games hub, or the Minecraft server itself: `infra/minecraft.tf`, agent-1 placement and hostPort, DNS, whitelist and ops, GriefPrevention and gamerules, RCON operations. | Info-only hub card with a Terraform drift test; Paper pod on agent-1 port 25565 via `mc.late.sh`; whitelist seeds plus `scripts/minecraft_whitelist_add.sh`; claims, griefing rules, and the snake_case gamerule gotcha. |
...
| `late-ssh/src/app/games/CONTEXT.md` | Shared game primitives used by the Arcade, the house tables, and daily games, especially cards, Late Chips, the `chess_core` chess kernel, or the `pool_core` billiard kernel. | Boundaries for shared card rendering, chip services, the surface-agnostic chess rules/board renderer, and the pool physics/rules/renderers (event-based billiard model with 3D spin, hand-written because no Rust crate can express draw and follow); use this for common primitives only, not Arcade or Lobby runtime/UI ownership. |
...
| `late-ssh/src/app/lobby/daily/CONTEXT.md` | Daily correspondence games: the open-challenge lobby, daily chess/battleship/connect-four/briscola/pool matches, the game roster (`DailyGame`), the `Ctrl+G` Lobby modal, the sidebar Lobby panel, the full-screen daily boards, move deadlines/forfeits, `/challenge`, or the `daily_matches` table. | Daily domain context: the `DailyGame` roster enum (per-game name/prize/reward key + add-a-game checklist), `DailyService` snapshot/events/sweeper, single-table challenge+match persistence with revision guard, battleship + connect four + briscola rules (briscola is the roster's first hidden-hand game: both hands live in the state and only the renderer keeps them hidden), eight-ball/nine-ball/snooker as the roster's only simulated games (the server re-simulates every shot from the stored rack, so they are the only games with no optimistic apply, and the shot animation is the daily board's only one), the three UI surfaces, per-match private chat + voice on the board, the backtick cycle, your-turn desktop notify, v1 scope boundaries, and future hooks (wagers, announcements). |
...
| `late-ssh/src/app/clubhouse/nightcap/CONTEXT.md` | Nightcap, the small bar out back of the Clubhouse (`n` or the back door to enter, Esc to leave): six stools, a hidden `kind='nightcap'` room only the seated may speak in and no bot ever answers in, a fixed drink menu on the tavern's chip and drunk rails, a muted TV, the tab board, lines carved into the stools (`nightcap_carvings`), `Screen::Nightcap`. | The hidden-room and no-AI contracts, the process-global `SharedSeats` and `SharedWall` contracts (single-replica, same as the Clubhouse), how orders and carvings settle off-thread, and the module map. |
...
- `BonsaiService` (in `app/bonsai/svc.rs`) is the one writer of `bonsai_trees`, and the row is the truth: every care action locks the row (`Tree::lock`), runs one pure `BonsaiState` rule over it, stores it, and sends `bonsai_changed` with the owner's id, all in one transaction, so sessions and replicas act one after the other on the same tree. The 200 daily chips are credited in that same transaction as the `last_watered` stamp, once per UTC day. Sessions hold a render mirror (`bonsai/session.rs`) fed by their own answers and by the notify (`start_listener_task`, the `CrownService` shape). See `late-ssh/src/app/bonsai/CONTEXT.md`.
...
| Tree (bonsai) | `bonsai_trees` | `user_id` UNIQUE, seed, last_watered DATE (the daily chip gate), is_alive, vigor, water_stress, last_simulated_date, branch_graph JSONB, selected_branch_id, mode, badge_glyph, planted_at, state_revision (counts stored writes; a session mirror uses it to drop an out-of-order copy). Created as `bonsai_v2_trees` (056), renamed and emptied by 177; the classic `bonsai_trees`, `bonsai_graveyard`, and `bonsai_daily_care` tables were dropped there. |
...
| ArtboardPiece / ArtboardPieceVote | `artboard_pieces`, `artboard_piece_votes` | Gallery pieces (migration 174): `user_id` FK, `title`, `width`/`height`, cropped `canvas`/`provenance` JSONB, `glyph_count`, `own_share_percent`, `content_hash` UNIQUE with `period_month`, `featured_on` (migration 188, UNIQUE among pieces still up: the day Sliding Puzzle showed it); applause rows `(piece_id, user_id)` PK with `author_user_id` denormalized for the no-self-applause CHECK. Model `late-core/src/models/artboard_piece.rs`. |
...
| Nightcap `SharedWall` | `state.rs` (`State.nightcap_house`) | One process-global wall (TV captions, tab board, carvings) refreshed by one task per process; a carve updates it in place | Same exception; a second replica would show a carve late until its next refresh, or needs the notify shape from §0 |
...
5. Failure: If the paired client disconnects, paired state disappears. If the CLI viz source disconnects or goes silent, the spectrum expires after 750ms and the eq falls back to the ambient band. If SSH disconnects, the session token unregisters on drop.
...
- **The CLI analyzes, the server only draws:** `viz` frames carry the paired CLI's FFT of its audible Icecast/radio output, and on Linux of the YouTube helper's PipeWire stream, which the CLI records itself. They update `AudioState`'s spectrum without dirtying the app, expire after 750ms of silence, and fall back to the wall-tick ambient band wherever no capture runs (YouTube on Windows/macOS, or without PipeWire tools); see `late-ssh/src/app/audio/CONTEXT.md` §10 and §18.
...
| **Games** | 3 | Active | Dedicated landing/launcher hub for the immersive door games. A selector row of game cards (Lateania, NetHack, DCSS, Brogue, Usurper, Green Dragon, A Dark Room, Rebels, dopewars, BashQuest, CodeKeep, plus the info-only Minecraft card for the server at `mc.late.sh`) renders the selected game's full landing below it; arrows or `h`/`l`/`j`/`k` switch cards, `Enter` launches the selected game (nothing on the Minecraft card), `Ctrl+J`/`Ctrl+K` or `Ctrl+Down`/`Ctrl+Up` scroll the selected landing, and `d` resets the Lateania character when Lateania is selected. Lives in `late-ssh/src/app/door/hub`. |
...
| arrows / `h`/`l`/`j`/`k` | Games hub (page 3) | Switch the selected door-game card (Lateania / Minecraft / NetHack / DCSS / Brogue / Usurper / Green Dragon / A Dark Room / Rebels / dopewars / BashQuest / CodeKeep) |
| `Enter` | Games hub | Launch the selected door game directly into its live screen; does nothing on the info-only Minecraft card |
| `Ctrl+J` / `Ctrl+K`, `Ctrl+Down` / `Ctrl+Up` | Games hub | Scroll the selected game's landing down / up, clamped to its length and reset when switching games. `Enter` matches CR only, because `Ctrl+J` is a bare LF |
...
- **The SSH terminal backend is `GlyphIsolatingBackend` (`app/terminal_backend.rs`), not a bare `CrosstermBackend`.** Its `draw` never trusts cursor contiguity across a cell that is not one ASCII byte: the next cell gets an absolute cursor move, and a cell the model sizes wide is blanked with spaces before the glyph is written. The rule is broad on purpose: box drawing and block elements are East Asian Ambiguous width and CJK-locale terminals draw them two wide, so a border row can shift like an emoji row. Its cost is kept down instead: after a single codepoint the cursor is at most a column off and still on its row, so the next cell on that row takes a column-only move (`CSI n G`); only a multi-codepoint grapheme, which a per-codepoint terminal can draw past the row end, forces the full row-and-column move. `terminal_backend_test` pins a bordered 160x40 repaint at about 3.5 KB against 1.9 KB stock; the real fix for the bytes and the bleed alike is a per-session width probe. unicode-width (a ZWJ flag is 2, a VS16 emoji is 2) and a per-codepoint wcwidth terminal (1, 3 or 4) disagree in ways the server cannot see; with a contiguous run that error shifts the rest of the row and the diff then never rewrites the stray tail (the "w]" stuck at line ends, gone only on Ctrl+L). Isolation confines it to the glyph's own cells. A terminal that draws a glyph *wider* than the model can still bleed into the following cell until that cell changes; fixing the glyph itself needs a per-session width probe (`CSI 6 n` after a sample glyph) and a substitution policy, not the backend.
